"""
Heritage Atlas — Intent Classification Model Training v4
Expanded multilingual dataset with Romanized Hindi, Marathi, Tamil, Punjabi support.
"""
...
ML_CSV = os.path.join(DATA_DIR, "romanized_multilingual.csv")
COMBINED_CSV = os.path.join(DATA_DIR, "combined_training_data_v4.csv")
TRAIN_SPLIT = os.path.join(DATA_DIR, "train_v4.csv")
TEST_SPLIT = os.path.join(DATA_DIR, "test_v4.csv")
MODEL_PATH = os.path.join(MODEL_DIR, "intent_classifier_v4.joblib")
METRICS_PATH = os.path.join(MODEL_DIR, "evaluation_metrics_v4.json")
...
def load_and_combine():
    ...
    # ML data already has correct columns
    ml_normalized = ml[["text", "language", "state", "intent"]].copy()

    combined = pd.concat([base, rg_normalized, ml_normalized], ignore_index=True)
    ...
    print(f"Romanized Multilingual: {len(ml)} samples")
    ...
    # Count romanized examples
    if "script" in combined.columns:
        roman_count = len(combined[combined["script"] == "roman"])
        print(f"\nRomanized entries: {roman_count}")
...
def split_data(df, test_size=0.2, random_state=42):
    ...
    # Check near-duplicates (same text lowercased)
    train_norm = set(train_df["text"].str.lower().str.strip())
    test_norm = set(test_df["text"].str.lower().str.strip())
    near_leakage = train_norm & test_norm
    if near_leakage:
        print(f"WARNING: {len(near_leakage)} near-duplicates!")
    else:
        print("No near-duplicates between train and test.")
...
def evaluate_model(pipeline, test_df):
    ...
    # Per-script (roman vs native)
    if "script" in test_df.columns:
        for script in test_df["script"].unique():
            mask = test_df["script"] == script
            if mask.sum() > 0:
                acc = accuracy_score(y_test[mask], y_pred[mask])
                print(f"  Script '{script}': {acc:.4f} ({mask.sum()} samples)")
                per_language[f"script_{script}"] = {"accuracy": float(acc), "samples": int(mask.sum())}
...
def main():
    print("=" * 60)
    print("HERITAGE ATLAS — Intent Classification Model Training v4")
    print("=" * 60)
